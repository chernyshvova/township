...
public class LifecycleRegistry extends Lifecycle {
        ...
        while (descendingIterator.hasNext() && !this.mNewEventOccurred) {
            ...
            while (observerWithState.mState.compareTo(this.mState) > 0 && !this.mNewEventOccurred && this.mObserverMap.contains(next.getKey())) {
                ...
                if (downFrom != null) {
                    ...
                    StringBuilder outline24 = GeneratedOutlineSupport.outline24("no event down from ");
                    outline24.append(observerWithState.mState);
    ...
    private void enforceMainThreadIfNeeded(String str) {
        if (this.mEnforceMainThread && !ArchTaskExecutor.getInstance().isMainThread()) {
            throw new IllegalStateException(GeneratedOutlineSupport.outline17("Method ", str, " must be called on the main thread"));
        }
    }
        ...
        while (iteratorWithAdditions.hasNext() && !this.mNewEventOccurred) {
            ...
            while (observerWithState.mState.compareTo(this.mState) < 0 && !this.mNewEventOccurred && this.mObserverMap.contains(entry.getKey())) {
                ...
                if (upFrom != null) {
                    ...
                    StringBuilder outline24 = GeneratedOutlineSupport.outline24("no event up from ");
                    outline24.append(observerWithState.mState);
                    throw new IllegalStateException(outline24.toString());
                }
            }
