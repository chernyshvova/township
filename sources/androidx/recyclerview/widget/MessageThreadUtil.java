    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(final ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new ThreadUtil.BackgroundCallback<T>() {
            ...
            public Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem next = C04222.this.mQueue.next();
                        if (next == null) {
                            C04222.this.mBackgroundRunning.set(false);
                            return;
                        }
                        int i = next.what;
                        if (i == 1) {
                            C04222.this.mQueue.removeMessages(1);
                            backgroundCallback.refresh(next.arg1);
                        } else if (i == 2) {
                            C04222.this.mQueue.removeMessages(2);
                            C04222.this.mQueue.removeMessages(3);
                            ...
                            StringBuilder outline24 = GeneratedOutlineSupport.outline24("Unsupported message, what=");
                            outline24.append(next.what);
                            Log.e("ThreadUtil", outline24.toString());
                        } else {
    ...
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(final ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new ThreadUtil.MainThreadCallback<T>() {
            ...
            public Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem next = C04201.this.mQueue.next();
                    while (next != null) {
                        int i = next.what;
                        if (i == 1) {
