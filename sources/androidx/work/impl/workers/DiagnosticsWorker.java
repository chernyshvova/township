...
public class DiagnosticsWorker extends Worker {
    ...
    public static String workSpecRow(@NonNull WorkSpec workSpec, @Nullable String str, @Nullable Integer num, @NonNull String str2) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", new Object[]{workSpec.f34id, workSpec.workerClassName, num, workSpec.state.name(), str, str2});
    }
    ...
    public static String workSpecRows(@NonNull WorkNameDao workNameDao, @NonNull WorkTagDao workTagDao, @NonNull SystemIdInfoDao systemIdInfoDao, @NonNull List<WorkSpec> list) {
        ...
        for (WorkSpec next : list) {
            Integer num = null;
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(next.f34id);
            if (systemIdInfo != null) {
                num = Integer.valueOf(systemIdInfo.systemId);
            }
            sb.append(workSpecRow(next, TextUtils.join(",", workNameDao.getNamesForWorkSpecId(next.f34id)), num, TextUtils.join(",", workTagDao.getTagsForWorkSpecId(next.f34id))));
        }
        return sb.toString();
    }
