...
public class CacheBustJob implements Job {
    ...
    public CacheBustJob(@NonNull VungleApiClient vungleApiClient, @NonNull Repository repository2, @NonNull ExecutorService executorService, AdLoader adLoader2) {
        ...
        try {
            ...
            if (placement != null) {
                ...
                if (placement.isMultipleHBPEnabled()) {
                    this.adLoader.loadEndlessIfNeeded(placement, placement.getAdSize(), 0);
                } else {
                    AdLoader adLoader2 = this.adLoader;
                    AdLoader.Operation operation = r5;
                    AdLoader.Operation operation2 = new AdLoader.Operation(new AdRequest(placement.getId()), placement.getAdSize(), 0, 2000, 5, 1, 0, false, placement.getAutoCachePriority(), new LoadAdCallback[0]);
                    adLoader2.load(operation);
                }
            }
    ...
    private void processBust(Iterable<CacheBust> iterable) {
        for (CacheBust next : iterable) {
            ...
            if (linkedList.isEmpty()) {
                ...
                while (it.hasNext()) {
                    bustAd((Advertisement) it.next(), next);
                }
            }
        }
    }
    ...
    private void sendAnalytics() {
        ...
        L_0x0049:
            ...
            com.vungle.warren.network.Call r0 = r0.bustAnalytics(r1)     // Catch:{ IOException -> 0x00a3 }
