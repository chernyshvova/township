...
public class AttachmentDataProvider {
    ...
    public static Intent getIntentForDocument(int i, List<String> list) {
        Intent intent;
        if (i == 2) {
        ...
        int i2 = 0;
        for (String str : mIMEForDocument) {
            strArr[i2] = str;
            i2++;
        }
    ...
    public static Intent getIntentForImage(Context context, int i) {
        return getIntentForMedia(context, "image/*", MediaStore.Images.Media.EXTERNAL_CONTENT_URI, i);
    }

    public static Intent getIntentForMedia(Context context, String str, Uri uri, int i) {
        if (i == 2) {
    ...
    public static Intent getIntentForVideo(Context context, int i) {
        return getIntentForMedia(context, "video/*", MediaStore.Video.Media.EXTERNAL_CONTENT_URI, i);
    }

    public static Set<String> getMIMEForDocument(List<String> list) {
        HashSet hashSet = new HashSet();
        for (String next : list) {
            if (!next.startsWith(AttachmentConstants.IMAGE_MIME_PREFIX) && !next.startsWith(AttachmentConstants.VIDEO_MIME_PREFIX)) {
                hashSet.add(next);
            }
        }
        hashSet.add(AttachmentConstants.UNKNOWN_FILE_MIME);
        return hashSet;
    }
}
